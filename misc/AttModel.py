...
class NewTopDownCore(nn.Module):
    ...
    def forward(self, xt, fc_feats, conv_feats, p_conv_feats, pool_feats, p_pool_feats, att_mask, pnt_mask, state):
        ...
        ########################################################################
        att_gate2 = F.sigmoid(self.h2h_1(h_att2+c_att2))  ##
        ...
        #h_att1 += h_att2
        #c_att1 *= c_att2
        ada_gate_point = F.sigmoid(self.i2h_4(lstm1_in+lstm2_in) + self.h2h_4(h2_lang))
        ...
        fake_box = F.dropout(ada_gate_point*F.tanh(c2_lang), 0.5, training=self.training)
