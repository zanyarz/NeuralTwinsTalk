...
class Attention2(nn.Module):
    ...
    def forward(self, h, att_feats, p_att_feats, mask):
        ...
        att_h = att_h.unsqueeze(1).expand_as(att)            # batch * att_size * att_hid_size
        dot = att + att_h                                # batch * att_size * att_hid_size
        dot = F.sigmoid(dot)                              # batch * att_size * att_hid_size
        dot = dot.view(-1, self.att_hid_size)               # (batch * att_size) * att_hid_size
        # dot = F.dropout(dot, 0.3, training=self.training)
...
class Attention1(nn.Module):
    ...
    def forward(self, h, att_feats, p_att_feats):
        ...
        att_size = att_feats.numel() // batch_size // self.rnn_size
        att = p_att_feats.view(-1, att_size, self.att_hid_size)
        att_h = F.sigmoid(self.h2att(h))
        #att_h = F.sigmoid(att_h)
        att_h = F.tanh(self.h2att2(att_h))
        att_h = F.sigmoid(self.h2att3(att_h))
        #att_h = F.sigmoid(att_h)
        att_h = F.tanh(self.att2att(att_h))
        att_h = att_h.unsqueeze(1).expand_as(att)            # batch * att_size * att_hid_size
        dot = att + att_h                                # batch * att_size * att_hid_size
        dot = F.tanh(dot)                              # batch * att_size * att_hid_size
        dot = dot.view(-1, self.att_hid_size)               # (batch * att_size) * att_hid_size
        ...
        #dot = F.tanh(dot)                           # (batch * att_size) * 1
        dot = dot.view(-1, att_size)                        # batch * att_size
        weight = F.softmax(dot, dim=1)                             # batch * att_size
        att_feats_ = att_feats.view(-1, att_size, self.rnn_size) # batch * att_size * att_feat_size
        att_res = torch.bmm(weight.unsqueeze(1), att_feats_).squeeze(1) # batch * att_feat_size
...
class NewTopDownCore(nn.Module):
    ...
    def forward(self, xt, fc_feats, conv_feats, p_conv_feats, pool_feats, p_pool_feats, att_mask, pnt_mask, state):
        ...
        ########################################################################
        att_gate1 = F.sigmoid(self.h2h_1(h_att1+c_att1)) ##
        ################APPLY LSTM
        h_lang, c_lang = self.lang_lstm(lstm1_in, (h_att1,(att_gate1*c_att1)))  ##
        #h_lang = F.sigmoid(self.h2h_1(h_lang))
        ...
        ########################################################################
        att_gate2 = F.sigmoid(self.h2h_2(h_att2+c_att2))  ##
        ...
        #h1_lang = F.sigmoid(self.h2h_6(h1_lang)) * c1_lang * att_gate2 ##
        ########################################################################
        #h_lang_t = torch.add(h_lang, h1_lang)
        #c_lang_t = torch.add(c1_lang, c_lang)
    
        #att_gate3 = F.sigmoid(self.h2h_1(h_lang_t+c_lang_t)) ##
        ...
        #h2_lang = F.sigmoid(self.h2h_7(h2_lang* c2_lang * att_gate3)) 

        #c_lang_t = c1_lang + c_lang + c2_lang/3
        #h_lang_t = h1_lang + h_lang + h2_lang/3
        #c_lang_t = self.h2h_2(c2_lang)
        #h_lang_t = self.h2h_3(h2_lang)
        #att_gate4 = F.tanh(self.i2h_1(lstm1_in) + self.h2h_1(c2_lang))
        #att_gate4 = att_gate4 + att_gate3
        #h3_lang, c3_lang = self.lang_lstm4(lstm1_in, (h2_lang,att_gate4*c2_lang))
        #att_gate5 = F.sigmoid(self.i2h_1(lstm1_in1) + self.h2h_1(c3_lang))
        #h4_lang, c4_lang = self.lang_lstm5(lstm1_in, (h3_lang, att_gate5*c3_lang))
        #att_gate6 = F.sigmoid(self.i2h_1(lstm1_in1) + self.h2h_1(c4_lang))
        #h5_lang, c5_lang = self.lang_lstm6(lstm1_in, (h4_lang, att_gate6*c4_lang))
       
        #att_gate7 = F.sigmoid(self.i2h_1(lstm1_in1) + self.h2h_1(c5_lang))                                                                                                     
        #h6_lang, c6_lang = self.lang_lstm7(lstm1_in, (h5_lang, att_gate7*c5_lang))
        #att_gate8 = F.sigmoid(self.i2h_1(lstm1_in1) + self.h2h_1(c6_lang)
        #h7_lang, c7_lang = self.lang_lstm8(lstm1_in, (h6_lang, att_gate8*c6_lang))

        #att_gate9 = F.sigmoid(self.i2h_1(lstm1_in1) + self.h2h_1(c6_lang)
        #h8_lang, c8_lang = self.lang_lstm9(lstm1_in, (h7_lang, att_gate9*c7_lang))

        #att_gate10 = F.sigmoid(self.i2h_1(lstm1_in1) + self.h2h_1(c8_lang)
        #h9_lang, c9_lang = self.lang_lstm10(lstm1_in, (h8_lang, att_gate10*c8_lang))
        
        #h_att1 += h_att2
        #c_att1 *= c_att2
        #ada_gate_point = F.sigmoid(self.i2h_4(lstm1_in) + self.h2h_4(_lang))
        output0 = F.dropout(h_lang, 0.5, self.training)
        #output0 = F.tanh(self.h2h_5(output0))
        output1 = F.dropout(h1_lang, 0.5, self.training)
        ada_gate_point = F.sigmoid(self.i2h_4(lstm1_in+lstm2_in) + self.h2h_4(output0+output1))
        #_out0 = F.dropout(c_lang, 0.5, self.training)
        #c_out1 = F.dropout(c1_lang, 0.5, self.training)
        #output1 = F.tanh(self.h2h_6(output1))
        #output2 = F.dropout(h2_lang, 0.8, self.training)
        #output2 = F.tanh(self.h2h_7(output2))
        ...
        #output = F.tanh(self.h2h_1(output))
        #output = F.tanh(self.h2h_2(output))
        #output = F.tanh(self.h2h_3(output))
        #output = F.tanh(self.h2h_4(output))
        ...
        fake_box = F.dropout(ada_gate_point*F.tanh(c_lang+c1_lang), 0.5, training=self.training)
        #fake_box = F.relu(ada_gate_point*F.tanh(c_lang_t))
        det_prob = self.adaPnt(output, fake_box, pool_feats, p_pool_feats, pnt_mask)
        ...
        #h_lang = self.h2h_2(h_lang)
        #h1_lang = self.h2h_2(h1_lang)
        #c_lang = self.h2h_3(c_lang)
        #c1_lang = self.h2h_3(c1_lang)
        h_lang = h_att1 + output0
        h1_lang = h_att2 + output1
        
        state = (torch.stack([h_lang, h1_lang]), torch.stack([c_att1, c_att1]))
...
class CascadeCore(nn.Module):
    def __init__(self, opt):
        ...
        self.bert_base = opt.bert_base_768
        self.glove = opt.glove_6B_300
    ...
    def forward(self, fg_idx, pool_feats, rnn_outs, roi_labels, seq_batch_size, seq_cnt):
        ...
        # fg_out 
        if self.bert_base:
            fg_score = torch.mm(fg_out.view(-1,768), self.fg_emb.t()).view(seq_batch_size, -1, self.fg_size+1)
