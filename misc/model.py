...
class AttModel(CaptionModel):
    def __init__(self, opt):
        ...
        if opt.bert_base_768:
            self.pool_feat_size = self.att_feat_size + 768 * 2
        if opt.glove_6B_300:
        ...
        if opt.bert_base_768:
            self.det_fc = nn.Sequential(nn.Embedding(self.detect_size+1, 768),nn.ReLU(),nn.Dropout())
            self.loc_fc = nn.Sequential(nn.Linear(5, 768),nn.ReLU(),nn.Dropout())
    ...
    def forward(self, img, seq, gt_seq, num, ppls, gt_boxes, mask_boxes, opt, eval_opt = {}):
        ...
        if opt == 'MLE':
            ...
            with torch.no_grad():
                greedy_result = self._sample( Variable(img.data), Variable(ppls.data),Variable(num.data), {'sample_max':1, 'beam_size':1, 'inference_mode' : False} )
    ...
    def _forward(self, img, seq, ppls, gt_boxes, mask_boxes, num):
        ...
        lm_loss = torch.unsqueeze(lm_loss, 0)
        bn_loss = torch.unsqueeze(bn_loss, 0)
        fg_loss = torch.unsqueeze(fg_loss, 0)
